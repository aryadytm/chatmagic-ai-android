...
class Tokenizer(
    ...
    fun getBadWordTokens(badWords: List<String>): List<Int> {
        val badWordTokens = mutableListOf<Int>()

        for (token in encoder.entries) {
            for (badWord in badWords) {

                if (token.key.lowercase().contains(badWord)) {
                    badWordTokens.add(token.value)
                }

            }
        }

        logDebug("Got ${badWordTokens.size} bad word tokens.")
        return badWordTokens
    }
